<?php $page_title="Manage Users Activation Code to Login";
    ...
    $tableName="tbl_activation_code";   
    $keyword='';

    if(!isset($_GET['keyword'])){
        $sql_query="SELECT * FROM tbl_activation_code ORDER BY tbl_activation_code.`id` DESC"; 
    } else {
        $keyword=addslashes(trim($_GET['keyword']));
        $sql_query="SELECT * FROM tbl_activation_code WHERE (`user_name` LIKE '%$keyword%' OR `activation_code` LIKE '%$keyword%') ORDER BY tbl_users.`id` DESC"; 
    }
                ...
                <div class="d-flex mt-2 mt-md-0">
                    <form method="get" id="searchForm" action="" class="me-2">
                        <div class="input-group">
                            <input type="text" id="search_input" class="form-control" placeholder="Search username or code" name="keyword" value="<?php if(isset($_GET['keyword'])){ echo $_GET['keyword'];} ?>" required="required">
                            <button class="btn btn-outline-default d-inline-flex align-items-center" type="search">
                                <i class="ri-search-2-line"></i>
                            </button>
                        </div>
                    </form>
                    <a href="create_user_activation.php?add=yes" class="btn btn-primary d-inline-flex align-items-center justify-content-center">
                                ...
                                <?php $i=0; while($row=mysqli_fetch_array($result)) { ?>
                                    <tr class="card-item">
                                        ...
                                        <td class="text-center display-desktop">
                                            <a href="create_user_activation.php?user_id=<?php echo $row['id']; ?>" class="btn btn-primary btn-icon" style="padding: 10px 10px !important;  margin-right: 10px !important;" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit">
