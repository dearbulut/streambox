<?php $page_title="Manage Custom Ads";
    ...
    if(!isset($_GET['keyword'])){
        $sql_query="SELECT * FROM tbl_custom_ads ORDER BY tbl_custom_ads.`id` DESC"; 
    } else{
        $keyword=addslashes(trim($_GET['keyword']));
        $sql_query="SELECT * FROM tbl_custom_ads WHERE (`ads_title` LIKE '%$keyword%') ORDER BY tbl_custom_ads.`id` DESC"; 
    }
                        ...
                        <?php $i=0; while($row=mysqli_fetch_array($result)) { ?>
                            <div class="col-lg-3 col-sm-6 card-item">
                                <div class="nsofts-card-light p-3">
                                    <h5 class="mb-2 text-truncate"><?php echo $row['ads_title'];?></h5>
