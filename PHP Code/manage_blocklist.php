<?php $page_title="Manage DNS Blocklist";
    ...
    if(!isset($_GET['keyword'])){
        $sql_query="SELECT * FROM tbl_xui_dns_block ORDER BY tbl_xui_dns_block.`id` DESC"; 
    } else{
        $keyword=addslashes(trim($_GET['keyword']));
        $sql_query="SELECT * FROM tbl_xui_dns_block WHERE (`dns_base` LIKE '%$keyword%') ORDER BY tbl_xui_dns_block.`id` DESC"; 
    }
                ...
                <div class="d-flex mt-2 mt-md-0">
                    <form method="get" id="searchForm" action="" class="me-2">
                        <div class="input-group">
                            <input type="text" id="search_input" class="form-control" placeholder="Search dns" name="keyword" value="<?php if(isset($_GET['keyword'])){ echo $_GET['keyword'];} ?>" required="required">
                            <button class="btn btn-outline-default d-inline-flex align-items-center" type="search">
                                <i class="ri-search-2-line"></i>
                            </button>
                        </div>
                    </form>
                    <a href="create_blocklist.php?add=yes" class="btn btn-primary d-inline-flex align-items-center justify-content-center">
                        ...
                        <?php $i=0; while($row=mysqli_fetch_array($result)) { ?>
                            <div class="col-lg-3 col-sm-6 card-item">
                                <div class="nsofts-card-light p-3">
                                    <h5 class="mb-3 text-truncate"><?php echo $row['dns_base'];?></h5>
                                    <div class="d-flex justify-content-between">
                                        <div class="d-flex">
                                           <a href="create_blocklist.php?dns_id=<?php echo $row['id'];?>" class="btn btn-outline-primary rounded-pill me-2 btn-icon" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit">
