<?php $page_title="Manage Notification ";
    ...
    if(!isset($_GET['keyword'])){
        $sql_query="SELECT * FROM tbl_notification ORDER BY tbl_notification.`id` DESC"; 
    } else{
        $keyword=addslashes(trim($_GET['keyword']));
        $sql_query="SELECT * FROM tbl_notification WHERE (`notification_title` LIKE '%$keyword%') ORDER BY tbl_notification.`id` DESC"; 
    }
                            ...
                            <tbody id="load-more-container">
                                <?php $i=0; while($row=mysqli_fetch_array($result)) { ?>
                                    <tr class="card-item">
                                        ...
                                        <td  class="display-desktop"><?php echo $row['notification_msg'];?></td>
