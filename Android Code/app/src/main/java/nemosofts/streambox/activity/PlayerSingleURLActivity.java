...
public class PlayerSingleURLActivity extends AppCompatActivity {
    ...
    private class PlayerListener implements Player.Listener {
        ...
        public void onPlayerError(@NonNull PlaybackException error) {
            ...
            if (playback < 5){
                playback = playback + 1;
                Toast.makeText(PlayerSingleURLActivity.this,"Playback error - "+ playback + "/5 ", Toast.LENGTH_SHORT).show();
                ...
                exoPlayer.stop();
                loadingProgressBar.setVisibility(View.GONE);
                Toast.makeText(PlayerSingleURLActivity.this,"Failed : " + error.getErrorCodeName(), Toast.LENGTH_SHORT).show();
            }
        }
    }

    public void startTimer() {
        if (!isFinishing() && !isTvBox && countDownTimer == null && Boolean.TRUE.equals(Callback.getRewardAdSingle())){
            countDownTimer = new CountDownTimer(timeLeftInMillis, 1000) {
                ...
                public void onFinish() {
                    ...
                    helper.showRewardAds(Callback.getRewardAdSingle(),exoPlayer != null && exoPlayer.isPlaying(), playWhenReady1 -> {
    ...
    public boolean onKeyDown(int keyCode, @NonNull KeyEvent event) {
        if (exoPlayer == null) {
            return super.onKeyDown(keyCode, event); // Early exit if exoPlayer is null
        }
        switch (keyCode) {
