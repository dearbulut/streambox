...
public class UiOneActivity extends AppCompatActivity {
    ...
    private static final String TAG = "OneUIActivity";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        OnBackPressedCallback callback = new OnBackPressedCallback(true) {
            ...
            public void handleOnBackPressed() {
                DialogUtil.exitDialog(UiOneActivity.this);
            }
        };
        ...
        progressDialog = new ProgressDialog(UiOneActivity.this, true);
        ...
        new Handler(Looper.getMainLooper()).postDelayed(() -> {
            ...
            DialogUtil.popupAdsDialog(UiOneActivity.this);
        }, 600);
    }
    ...
    private void setAds() {
        ...
        new GDPRChecker(UiOneActivity.this).check();
    ...
    private void setListenerHome() {
        findViewById(R.id.iv_notifications).setOnClickListener(v -> startActivity(new Intent(UiOneActivity.this, NotificationsActivity.class)));
        findViewById(R.id.iv_file_download).setOnClickListener(v -> startActivity(new Intent(UiOneActivity.this, DownloadActivity.class)));
        findViewById(R.id.iv_profile).setOnClickListener(v -> startActivity(new Intent(UiOneActivity.this, ProfileActivity.class)));
        findViewById(R.id.iv_profile_re).setOnClickListener(v -> signOut());
        findViewById(R.id.iv_settings).setOnClickListener(v -> startActivity(new Intent(UiOneActivity.this, SettingActivity.class)));
        ...
        findViewById(R.id.select_epg).setOnClickListener(v -> {
            if (isDownloadLive()) {
                startActivity(new Intent(UiOneActivity.this, CategoriesActivity.class));
            }
        });
        findViewById(R.id.select_multiple_screen).setOnClickListener(v -> {
            if (isDownloadLive()) {
                startActivity(new Intent(UiOneActivity.this, MultipleScreenActivity.class));
            }
        });
        findViewById(R.id.select_catch_up).setOnClickListener(v -> {
            if (isDownloadLive()) {
                startActivity(new Intent(UiOneActivity.this, CatchUpActivity.class));
            }
        });
        findViewById(R.id.iv_radio).setOnClickListener(v -> {
            if (isDownloadLive()) {
                startActivity(new Intent(UiOneActivity.this, RadioActivity.class));
            }
        });
    ...
    private void setAutoRenew() {
        findViewById(R.id.ll_tv_auto_renew).setOnClickListener(v -> {
            if (!spHelper.getCurrent(Callback.TAG_TV).isEmpty()) {
                DialogUtil.downloadDataDialog(UiOneActivity.this, Callback.TAG_TV, type -> getLive());
            } else {
                getLive();
            }
        });
        findViewById(R.id.ll_movie_auto_renew).setOnClickListener(v -> {
            if (!spHelper.getCurrent(Callback.TAG_MOVIE).isEmpty()) {
                DialogUtil.downloadDataDialog(UiOneActivity.this, Callback.TAG_MOVIE, type -> getMovies());
            } else {
                getMovies();
            }
        });
        findViewById(R.id.ll_series_auto_renew).setOnClickListener(v -> {
            if (!spHelper.getCurrent(Callback.TAG_SERIES).isEmpty()) {
                DialogUtil.downloadDataDialog(UiOneActivity.this, Callback.TAG_SERIES, type -> getSeries());
            } else {
                getSeries();
            }
        });
    ...
    private void setLongClickListener() {
        ...
        findViewById(R.id.select_movie).setOnLongClickListener(v -> {
            if (!spHelper.getCurrent(Callback.TAG_MOVIE).isEmpty()) {
                DialogUtil.downloadDataDialog(UiOneActivity.this, Callback.TAG_TV, type -> getMovies());
            }
            return false;
        });
        findViewById(R.id.select_serials).setOnLongClickListener(v -> {
            if (!spHelper.getCurrent(Callback.TAG_SERIES).isEmpty()) {
                DialogUtil.downloadDataDialog(UiOneActivity.this, Callback.TAG_TV, type -> getSeries());
            }
            return false;
        });
    }
    ...
    private void selectSerials() {
        if (spHelper.getCurrent(Callback.TAG_SERIES).isEmpty()) {
            ...
            startActivity(new Intent(UiOneActivity.this, SeriesActivity.class));
        }
    }
    ...
    private void selectMovie() {
        if (spHelper.getCurrent(Callback.TAG_MOVIE).isEmpty()) {
            ...
            startActivity(new Intent(UiOneActivity.this, MovieActivity.class));
        }
    }
    ...
    private void selectLive() {
        if (spHelper.getCurrent(Callback.TAG_TV).isEmpty()) {
            ...
            startActivity(new Intent(UiOneActivity.this, LiveTvActivity.class));
        }
    }
    ...
    private void signOut() {
        DialogUtil.logoutDialog(UiOneActivity.this, () -> {
            Intent intent = new Intent(UiOneActivity.this, UsersListActivity.class);
            if (spHelper.isLogged()) {
                new JSHelper(UiOneActivity.this).removeAllData();
                spHelper.removeSignOut();
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
                intent.putExtra("from", "");
                Toast.makeText(UiOneActivity.this, getString(R.string.logout_success), Toast.LENGTH_SHORT).show();
            } else {
    ...
    private void getSeries() {
        if (!NetworkUtils.isConnected(this)){
            pbSerials.setVisibility(View.GONE);
            Toasty.makeText(UiOneActivity.this,true, getString(R.string.err_internet_not_connected), Toasty.ERROR);
            return;
        }
    ...
    private void handlerSeriesEnd(String success, String msg) {
        if (success.equals("1")) {
            ...
            spHelper.setCurrentDate(Callback.TAG_SERIES);
            changeIconView(spHelper.getCurrent(Callback.TAG_SERIES).isEmpty(), Callback.TAG_SERIES,false);
            Toast.makeText(UiOneActivity.this, getString(R.string.added_success), Toast.LENGTH_SHORT).show();
        }  else {
            spHelper.setCurrentDateEmpty(Callback.TAG_SERIES);
            changeIconView(spHelper.getCurrent(Callback.TAG_SERIES).isEmpty(), Callback.TAG_SERIES,true);
            pbSerials.setVisibility(View.GONE);
            if (success.equals("3")){
                Toasty.makeText(UiOneActivity.this,true, msg, Toasty.ERROR);
            } else {
                Toast.makeText(UiOneActivity.this, getString(R.string.err_server_not_connected), Toast.LENGTH_SHORT).show();
            }
        }
    }
